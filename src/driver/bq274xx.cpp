...
const struct bq274xx_regs bq27421_regs = {
...
const struct bq274xx_regs bq27427_regs = {
...
int bq274xx_ctrl_reg_write(const struct device *dev, uint16_t subcommand)
{
	const struct bq274xx_config *config = dev->config;
	int ret;
...
int bq274xx_get_device_type(const struct device *dev, uint16_t *val)
{
	...
	ret = bq274xx_cmd_reg_read(dev, BQ274XX_CMD_CONTROL, (int16_t*) val);
	if (ret < 0) {
		LOG_ERR("Unable to read register\n");
		return -EIO;
	}
...
int bq274xx_read_block(const struct device *dev,
			      uint8_t subclass,
...
int bq274xx_write_block(const struct device *dev,
			       uint8_t *block, uint8_t num_bytes)
{
	const struct bq274xx_config *const config = dev->config;
	uint8_t checksum = 0;
...
void bq274xx_update_block(uint8_t *block,
...
int bq274xx_mode_cfgupdate(const struct device *dev, bool enabled)
{
	uint16_t flags;
	uint8_t _try;
	int ret;
	...
	if (ret < 0) {
		LOG_ERR("Unable to set device mode to %x", val);
		return -EIO;
	}

	for (_try = 0; _try < BQ274XX_CFGUP_MAX_TRIES; _try++) {
		ret = bq274xx_cmd_reg_read(dev, BQ274XX_CMD_FLAGS, (int16_t*)&flags);
		if (ret < 0) {
		...
		if (enabled_flag == enabled) {
			LOG_DBG("CFGUP ready, _try %u", _try);
			break;
		}
	...
	if (_try >= BQ274XX_CFGUP_MAX_TRIES) {
 ...
 */
int bq27427_ccgain_quirk(const struct device *dev)
{
	const struct bq274xx_config *const config = dev->config;
	int ret;
...
int bq274xx_ensure_chemistry(const struct device *dev)
{
...
int bq274xx_channel_get(const struct device *dev, enum sensor_channel chan,
...
int bq274xx_sample_fetch(const struct device *dev, enum sensor_channel chan)
{
	...
	if (chan == SENSOR_CHAN_ALL || chan == SENSOR_CHAN_GAUGE_TEMP) {
		ret = bq274xx_cmd_reg_read(dev, BQ274XX_CMD_INT_TEMP,
			(int16_t*)&data->internal_temperature);
		if (ret < 0) {
	...
	if (chan == SENSOR_CHAN_ALL || chan == SENSOR_CHAN_GAUGE_FULL_CHARGE_CAPACITY) {
		ret = bq274xx_cmd_reg_read(dev, BQ274XX_CMD_FULL_CAPACITY,
			(int16_t*)&data->full_charge_capacity);
		if (ret < 0) {
	...
	if (chan == SENSOR_CHAN_ALL || chan == SENSOR_CHAN_GAUGE_REMAINING_CHARGE_CAPACITY) {
		ret = bq274xx_cmd_reg_read(dev, BQ274XX_CMD_REM_CAPACITY,
			(int16_t*)&data->remaining_charge_capacity);
		if (ret < 0) {
	...
	if (chan == SENSOR_CHAN_ALL || chan == SENSOR_CHAN_GAUGE_NOM_AVAIL_CAPACITY) {
		ret = bq274xx_cmd_reg_read(dev, BQ274XX_CMD_NOM_CAPACITY,
			(int16_t*)&data->nom_avail_capacity);
		if (ret < 0) {
	...
	if (chan == SENSOR_CHAN_ALL || chan == SENSOR_CHAN_GAUGE_FULL_AVAIL_CAPACITY) {
		ret = bq274xx_cmd_reg_read(dev, BQ274XX_CMD_AVAIL_CAPACITY,
			(int16_t*)&data->full_avail_capacity);
		if (ret < 0) {
...
int bq274xx_enter_shutdown_mode(const struct device *dev)
{
	int ret;
...
int bq274xx_exit_shutdown_mode(const struct device *dev)
{
	const struct bq274xx_config *const config = dev->config;
	int ret;
...
int bq274xx_pm_action(const struct device *dev,
...
/*
DEVICE_API(sensor, bq274xx_battery_driver_api) = {
...
#define BQ274XX_INIT(index)							\
	struct bq274xx_data bq274xx_driver_##index;			\
										\
	const struct bq274xx_config bq274xx_config_##index = {		\
